...
//考虑到target[0] target[1] target[2] 在triplets的所有三元组中，必须与数字小于等于它们的三元组进行max操作，
//而且该三元组所有元必须同时小于等于它们，如果与任一元大于目标值target操作，得到的三元组则存在某元大于target，不会得到target
//最后得到的res三元组也要等于target三元组才行，判断返回结果
class Solution {
public:
	bool mergeTriplets(vector<vector<int>>& triplets, vector<int>& target) {
		vector<int> res(3, 0);
		for (auto &nums : triplets)
		{
